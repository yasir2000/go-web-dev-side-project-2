...
From now on, it is assumed that you will sort out the appropriate importstatements yourself.
...
Note: Ranging over a string returns the index of each character and a rune type, which is a numerical value (specifically, int32) representing the character itself. For more information about runes, characters, and strings, refer to http://blog.golang.org/strings.
...
This describes the slice from the beginning of the word until the selected vowel. The +1 is required because the value following the colon does not include the specified index; rather, it slices up to that value.
...
blueprints
bluprints
Let's see how Coolify plays with Sprinkle and Domainify by adding their names to our pipe chain. In the terminal, navigate back (using the cdcommand) to the parent folder and run the following commands:
...
./synonyms/synonyms | ./sprinkle/sprinkle | ./coolify/coolify |  ./domainify
